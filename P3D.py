...
class Bottleneck():
    ...
    def ST_B(self,name,x):
        ...
        tmp_x=tf.contrib.layers.batch_norm(tmp_x,is_training=IS_TRAIN)
        tmp_x=tf.nn.relu(tmp_x)
        x=convT(name+'_T',x,self.planes,self.planes)
        x=tf.contrib.layers.batch_norm(tmp_x,is_training=IS_TRAIN)
        x=tf.nn.relu(x)
        return x+tmp_x
    ...
    def infer(self):
        ...
        if len(self.downsample)==1:
            ...
            residual=tf.contrib.layers.batch_norm(residual,is_training=IS_TRAIN)
...
def inference_p3d(_X,_dropout,BATCH_SIZE):
    ...
    conv1_custom_bn=tf.contrib.layers.batch_norm(conv1_custom,is_training=IS_TRAIN)
    ...
    x=b1.infer()
    #cnt records current depth of p3d which is about to be used by next block.
    cnt=b1.cnt
